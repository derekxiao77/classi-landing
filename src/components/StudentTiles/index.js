...
function ContentFriends() {
  return (
    <div>
      ...
      <span>
        Workout and hangout with friends at the same time. Turn any live stream
        into a video chat by creating a private group and inviting your friends.
      </span>
    </div>
  );
}
...
function ContentFree() {
  return (
    <div>
      ...
      <span>
        Classi users can watch as many live classes as they want for free. For
        more flexibility, Classi Premium gives users full access to watch any
        past class and save classes to watch again in the future.
      </span>
    </div>
  );
}
...
function ContentGoals() {
  return (
    <div>
      ...
      <span>
        Classi offers a wide range of wellness classes from home workouts to
        home cooking. Reaching your fitness goals is now as simple as pressing
        play.
      </span>
    </div>
  );
        ...
        <div style={{ marginLeft: "5%" }}>
          ...
          <div className={styles.studentContentContainerMobile}>
            ...
            <div className={styles.tilesContainer}>
              <ValueTile
                image={friendsIcon}
                valueText="Watch with friends"
                thisTile={tileState.Friends}
                selectedTile={this.state.content}
                onClick={this.friendsClick}
              />
              <ValueTile
                image={clapIcon}
                valueText="Any live class. Free."
                thisTile={tileState.Free}
                selectedTile={this.state.content}
                onClick={this.freeClick}
              />
              <ValueTile
                image={praiseIcon}
                valueText="Reach your fitness goals"
                thisTile={tileState.Goals}
                selectedTile={this.state.content}
                onClick={this.goalsClick}
              />
            </div>
          </div>
        </div>
