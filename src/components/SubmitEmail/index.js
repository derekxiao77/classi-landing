...
const CustomForm = ({ status, message, onValidated }) => {
  ...
  if (status === "sending") {
    inputText = (
      <div className={styles.confirmationText} style={{ color: "#334E68" }}>
        Please wait one second, we are adding you to the list for exclusive
        access...
      </div>
    );
    ...
    inputText = (
      <div className={styles.confirmationText} style={{ color: "#5C913B" }}>
        Thank you for signing up! We will contact you soon with details for
        exclusive access.
      </div>
    );
  }
